...
<div class="uk-card uk-card-default uk-card-body uk-margin" uk-sticky="offset: 20; bottom: true">  
  ...
  <div v-if="price > 0">
    ...
    <table class="uk-table uk-table-striped uk-table-small uk-table-responsive">
        ...
        <tbody>
            <tr v-for="item in selectedDishes"  v-bind:key="item">
                <td class="uk-width-1-2">{{ item.Name }}</td>
                <td class="uk-table-shrink">{{ item.Price }}€</td>
                <td class="uk-table-shrink">{{ item.quantity }}</td>
                <td>
                  <a class="uk-margin-left"><span class="uk-badge" @click="addToCart(item)">+</span></a>
                  <a><span class="uk-badge" style="background: #f0506e;" @click="removeFromCart(item)">-</span></a>
                </td>
    ...
    <button class="uk-button uk-button-primary" @click="goToCheckout" name="button">Process to checkout ({{ price }} AED )</button> 
  </div>
